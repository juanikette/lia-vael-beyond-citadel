...
### Phase 10 - Persistent hybrid index and incremental scan
...
1. Consult LegendaryExplorer and document the first non-`BioConversation` container candidates for semantic extraction.
2. Implement the first minimal semantic container parser and wire it into evidence usage priority ahead of raw fallback.
3. Re-run the three narrative probes and record semantic vs fallback linkage metrics in Notion.
